...
class RLAgent:
    ...
    def build_model(columns, rows) -> Model:
        ...
        model.add(Dense(20, input_dim=columns * rows,
                        activation='relu'))  # input = state + categorical actions taken in 1-hot notation
    ...
    def run_training_session(self, episodes=100, versus="random", model_weights_file_suffix="",
        ...
        for episodeNo in range(episodes):
            ...
            while not done:
                ...
                cumreward += reward
                if done:
                    outcomestr = "draw" if reward == 0 else "won" if reward > 0 else "lose"
                    self.logger_.info(
...
for generation in range(1,20):
    ...
    logging.info(f"Generation {generation} outcomes")
